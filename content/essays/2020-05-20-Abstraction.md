...
I've opted to think of them as considerations, rather than standalone definitions, because what matters is not what some standalone code might be, but the sort of code it leads to. For example, in any object-oriented language you can define some 'abstract' Interface and then have a 'concrete' Class inherit it. Does that make an Interface an abstraction? If you think of it in terms of a standalone act (defining some model without a specific implementation), it might be; but if you think of it in terms of the code that arises from it (only being able to inherit properties, not compose them), I'd wager to say it never is. 

Another drawback of standalone definitions is that they define concretions and abstractions as opposites. So concretions, at least from what I picked up in Hickey's talks, are mainly talked about as something negative. But examining their role in my own programs, concretions are complementary to abstractions. Continuing with the above hierarchical entities example, first we had the implementation detail, `organization.parentId === null`, then the concretion, `isRootOrganization(org)`, and finally, the abstraction `isRoot(entity)`. Each step lead to what I'd consider better code, and I'm not sure I could have smoothly arrived at one without the other.

Interestingly, in being complementary, it seems like the goals and byproducts of concretions and abstraction are inverted. With a concretion, the goal is declarativeness, and code reuse (if it happens) is a byproduct. Sometimes a concretion saves lines of code, sometimes it doesn't, but you wrap some code anyway to better specify some behavior. Whereas with an abstraction, the goal is code reuse, and declarativeness (due to there being less imperative code) is a byproduct. The resulting abstraction is less declarative than its concretion, but that trade-off is outweighed by the reuse that comes from more generalized logic. With both, you reduce the complexity of your codebase and make it easier to reason about, but you do it through different means.

There's one exception I can think of, however, that hinders the goal of an abstraction. Sometimes you make a concretion, not for declarativeness, but for performance. Rather than have a data-structure cater to many use-cases, you have it maximize one. In Clojure, for instance, the function `map` can operate on any collection--i.e. lists, vectors, maps, and sets--but there's also a more specific version, `mapv`, that returns a vector specifically and doesn't pay any performance cost converting to and from a sequence. But, as always, performance concerns are special exceptions, and I still think it'd be useful to think of concretions as a tool for declarativeness and abstractions as a tool for reuse. With these clear boundaries, when you're reaching for one and not the other, it's easier to examine why.
